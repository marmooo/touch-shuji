...
function getKakusu(object, kanjiId) {
  var id = 'z' + kanjiId + 'd';
  var paths = object.contentDocument.querySelectorAll('[id^="' + id + '"]');
  var ids = [...paths].map(path => path.getAttribute('id'));
  var kakuIds = ids.map(id => {
...
function getTehonCanvas(object, kanjiId, kakusu, kakuNo) {
  return new Promise(function(resolve, reject) {
    ...
    for (var j=1; j<=kakusu; j++) {
      var kakuPaths = getKakuPaths(clonedContent, kanjiId, j);
      if (kakuNo != j) {
...
const specials = [
  '20043', '21450', '27665', '29995', '30001',
  '30002', '30003', '30011', '31899', '37325', '37340'];
...
function fetchKanjiSVG(kanji, kanjiId) {
  return new Promise((resolve, reject) => {
    fetch(animCJKDir + '/' + getKanjiDir(kanji, kanjiId) + '/' + kanjiId + '.svg')
...
function loadSVG(kanji, kanjiId, parentNode, pos, loadCanvas) {
  ...
  if (loadCanvas) {
    object.setAttribute('onload', 'initTegakiSVG(this)');
  } else {
    object.setAttribute('onload', 'initTehonSVG(this)');
  }
...
function changePathColor(pos, object, kanjiId, color) {
  ...
  var paths = getKakuPaths(object.contentDocument, kanjiId, pos);
  paths.forEach(path => {
    path.style.fill = color;
  });
}
...
function removeAnimations(object) {
  var animations = object.contentDocument.querySelectorAll('path[clip-path]');
  [...animations].forEach(animation => {
    animation.remove();
  });
}

function initTegakiSVG(object) {
  var kanjiId = object.dataset.id;
  var kakusu = getKakusu(object, kanjiId);
  removeAnimations(object);
  toggleStroke(object, kanjiId, kakusu);
